...
ctgov <- fread("./data/ctg-studies-all-completed-terminated-2024-10-24.csv", sep = ',')
total_n <- ctgov %>% nrow() 

# Include only interventional
study_type <- ctgov %>% group_by(`Study Type`) %>% summarise(n=n())
...
eligibility_year <- ctgov %>% group_by(eligible_year) %>% summarise(n=n())
ctgov <- ctgov %>% filter(eligible_year == T)

# Create bins for year grouping
ctgov <- ctgov %>%
  mutate(year_bin = cut(completion_year, breaks = c(-Inf, seq(2005, 2025, by = 5)), 
                        right = FALSE, 
                        labels = c(
                          "<2005",
...
write.csv2(df, file="../prototype/final-sample-ctgov.csv", row.names = F)
