...
const FILE_PATH = `./out/batch/gpt_job_query/output/${FILE_NAME}.jsonl`;

const OUTPUT_FOLDER = "./batch_results/queries/";
...
(async () => {
  ...
  const maxTokenObj = results.reduce((maxObj, obj) => {
    return (obj.response?.body?.usage?.total_tokens || 0) >
      (maxObj.response?.body?.usage?.total_tokens || 0)
      ? obj
      : maxObj;
  }, {});

  console.log("Result with the highest token count:");
  console.log(
    `Total Tokens: ${maxTokenObj.response?.body?.usage?.total_tokens}`
  );
