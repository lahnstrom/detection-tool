...
TrialScout is an automated tool for discovering publications linked to clinical trial registrations and detecting whether they contain trial results, using LLMs. Developed at Karolinska Institutet. This repo is the deployment repo — it contains the Node.js tool (`tool/`) and deployment infrastructure (nginx config, PM2, `deploy.sh`).
...
The server runs on a DigitalOcean droplet behind Nginx + PM2.

```bash
./deploy.sh              # Pull, install, restart PM2
./deploy.sh --setup      # First-time: install PM2, configure startup
./deploy.sh --with-frontend  # Also build and deploy vis-nordic frontend
```

- **PM2 process name:** `trialscout`
- **Entry point:** `tool/src/runner/server.js`
- **Port:** 3001 (proxied via nginx at `/api/`)
- **Nginx config:** `nginx/default`
- **Frontend:** served from `/var/www/metaresearch/`
